...
class ElevationLevel(enum.Enum):
    ...
    def scaled_radius(self) -> int:
        ...
        """
        return dp_module.dp(self.shadow_radius)
    ...
    def scaled_offset_y(self) -> int:
        ...
        """
        return dp_module.dp(self.shadow_offset_y)
...
def hex_color_to_qcolor(hex_str: str) -> QtGui.QColor:
    ...
    Returns:
        A valid ``QColor``.
    """
    tmp_raw = hex_str.lstrip("#")
    if len(tmp_raw) == 8:
        tmp_r = int(tmp_raw[0:2], 16)
        tmp_g = int(tmp_raw[2:4], 16)
        tmp_b = int(tmp_raw[4:6], 16)
        tmp_a = int(tmp_raw[6:8], 16)
        return QtGui.QColor(tmp_r, tmp_g, tmp_b, tmp_a)
...
def _bezier(
    ...
    """
    tmp_curve = QtCore.QEasingCurve(QtCore.QEasingCurve.Type.BezierSpline)
    tmp_curve.addCubicBezierSegment(
...
def _build_tokens(mode: ThemeMode) -> Win11Tokens:
    ...
    """
    tmp_reg = QtGui.QFont.Weight.Normal
    tmp_semi = QtGui.QFont.Weight.DemiBold
    tmp_lin = QtCore.QEasingCurve(QtCore.QEasingCurve.Type.Linear)
...
_LIGHT_TOKENS: Win11Tokens = _build_tokens(ThemeMode.Light)
_DARK_TOKENS: Win11Tokens = _build_tokens(ThemeMode.Dark)
...
def _detect_mode() -> ThemeMode:
    ...
    if not isinstance(tmp_app, QtWidgets.QApplication):
        return ThemeMode.Light
    tmp_window_color = tmp_app.palette().color(QtGui.QPalette.ColorRole.Window)
    if tmp_window_color.lightness() < 128:
...
def _resolve_platform_font() -> str:
    ...
    tmp_families = set(QtGui.QFontDatabase.families())
    if sys.platform == "win32":
        if "Segoe UI Variable" in tmp_families:
            return "Segoe UI Variable"
        return "Segoe UI"
    for tmp_candidate in ("Roboto", "DejaVu Sans"):
        if tmp_candidate in tmp_families:
...
def _ensure_initialized() -> None:
    ...
    tmp_app.installEventFilter(_filter_obj)
...
def make_font(
    ...
    tmp_font = QtGui.QFont(_platform_font_family)
    # setPixelSize expects physical pixels. dp() converts the dp token value.
    tmp_font.setPixelSize(dp_module.dp(size))
    tmp_font.setWeight(weight)
    return tmp_font
...
class TokenConsumer:
    """Mixin class for widgets that consume design tokens.

    This mixin provides automatic theme/mode synchronization. Any class
    inheriting from both ``QWidget`` and ``TokenConsumer`` must use cooperative
    multiple inheritance and call ``super().__init__()`` in both constructor
    chains. Because ``QWidget`` does not call ``super().__init__()`` in its C++
    ...
    Example:
        class CustomWidget(TokenConsumer, QtWidgets.QWidget):
            ...
            def _apply_tokens(self) -> None:
                tmp_tok = tokens()
                # Apply token values here
    """

    def __init__(self, *args: Any, **kwargs: Any) -> None:
        """Initialize the token consumer, connecting it to theme changes.

        Args:
            *args: Positional arguments forwarded to super class.
            **kwargs: Keyword arguments forwarded to super class.
        """
    ...
    def _apply_tokens(self) -> None:
        """Apply the current tokens to the widget.

        This method must be implemented by subclasses to update their visual
        styles.

        Raises:
            NotImplementedError: If not overridden by a subclass.
        """
        raise NotImplementedError("Subclasses must implement _apply_tokens.")

    def _connect_theme(self) -> None:
        """Register the token application callback on theme changes."""
        on_mode_changed(self._apply_tokens)
...
def _derive_accent_colors(
    base_color: QtGui.QColor,
    mode: ThemeMode,
) -> tuple[
    QtGui.QColor, QtGui.QColor, QtGui.QColor, QtGui.QColor, QtGui.QColor
]:
    """Derive accent colors from a base color for the given mode.

    Args:
        base_color: The base accent color.
        mode: The theme mode.

    Returns:
        A tuple of (default, hover, pressed, disabled, text_light) QColors.
    """
    tmp_h, tmp_s, tmp_l, tmp_a = base_color.getHslF()
    tmp_h = max(0.0, min(1.0, tmp_h))
    tmp_s = max(0.0, min(1.0, tmp_s))
    tmp_l = max(0.0, min(1.0, tmp_l))
    tmp_a = max(0.0, min(1.0, tmp_a))

    if mode == ThemeMode.Light:
        tmp_hover = QtGui.QColor.fromHslF(
            tmp_h, max(0.0, tmp_s - 0.23), min(1.0, tmp_l + 0.06), tmp_a
        )
        tmp_pressed = QtGui.QColor.fromHslF(
            tmp_h, tmp_s, max(0.0, tmp_l - 0.06), tmp_a
        )
        tmp_disabled = QtGui.QColor(0, 0, 0, 0x5C)
        tmp_text_light = QtGui.QColor.fromHslF(
            tmp_h, tmp_s, max(0.0, tmp_l - 0.09), tmp_a
        )
    else:
        tmp_hover = QtGui.QColor.fromHslF(
            tmp_h, tmp_s, min(1.0, tmp_l + 0.04), tmp_a
        )
        tmp_pressed = QtGui.QColor.fromHslF(
            tmp_h, tmp_s, max(0.0, tmp_l - 0.07), tmp_a
        )
        tmp_disabled = QtGui.QColor(255, 255, 255, 0x5C)
        tmp_text_light = base_color

    return base_color, tmp_hover, tmp_pressed, tmp_disabled, tmp_text_light


def register_accent(color: QtGui.QColor) -> None:
    """Replace the accent color family in the cached token instances.

    Rebuilds only the accent fields and fires ``on_mode_changed`` callbacks so
    every connected widget refreshes.

    Args:
        color: The new base accent color.
    """
    global _LIGHT_TOKENS, _DARK_TOKENS  # noqa: PLW0603
    ...
    (
        ...
        tmp_l_dis,
        tmp_l_tx,
    ) = _derive_accent_colors(color, ThemeMode.Light)
    _LIGHT_TOKENS = dataclasses.replace(
        _LIGHT_TOKENS,
        accent_default=tmp_l_def,
        accent_hover=tmp_l_hov,
        accent_pressed=tmp_l_prs,
        accent_disabled=tmp_l_dis,
        accent_text_light=tmp_l_tx,
    )
    ...
    (
        ...
        tmp_d_dis,
        tmp_d_tx,
    ) = _derive_accent_colors(color, ThemeMode.Dark)
    _DARK_TOKENS = dataclasses.replace(
        _DARK_TOKENS,
        accent_default=tmp_d_def,
        accent_hover=tmp_d_hov,
        accent_pressed=tmp_d_prs,
        accent_disabled=tmp_d_dis,
        accent_text_light=tmp_d_tx,
    )
