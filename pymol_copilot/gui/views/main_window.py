...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        self._menu_builder.add_action(self._save_menu_obj, "Save Session", "cmd_string", "save")
        self._menu_builder.add_action(self._save_menu_obj, "Save Session As...", "cmd_string", "save")
...
class PyMOLMenuBuilder:
    ...
    def add_action(self, menu, text, action_type: str, data: any = None) -> QtWidgets.QAction:
    ...
    def add_check_action(self, menu, text, setting_name: str, on_val=1, off_val=0) -> QtWidgets.QAction:
    ...
    def add_radio_group(self, menu, options, setting_name: str) -> QtGui.QActionGroup:
    ...
    def _dispatch_action(self) -> None:
        ...
        if action_type == "cmd_string":
            ...
            m = action.data()
            self.cmd.config_mouse(m) if ("config" in m or "two_button" in m) else self.cmd.mouse(m)
            ...
            self.cmd.set(setting_name, str(on_val if action.isChecked() else off_val), quiet=0)
    ...
    def build_more_menu(self, parent_menu):
        sys_browser_cmd = (
            "explorer ." if sys.platform == "win32"
        ...
        self.add_action(new_window_menu, "Ignore .pymolrc and plugins (-k)", "cmd_string", "new_window -k")
        ...
        self.add_action(wd_menu, "File Browser", "cmd_string", f"system {sys_browser_cmd}")

        parent_menu.addSeparator()
        self.add_action(parent_menu, "Edit pymolrc", "cmd_string", "edit_pymolrc")
        parent_menu.addSeparator()

        reinit_menu = parent_menu.addMenu("Reinitialize")
        self.add_action(reinit_menu, "Everything", "cmd_string", "reinitialize")
        self.add_action(reinit_menu, "Original Settings", "cmd_string", "reinitialize original_settings")
        self.add_action(reinit_menu, "Stored Settings", "cmd_string", "reinitialize settings")
        reinit_menu.addSeparator()
        self.add_action(reinit_menu, "Store Current Settings", "cmd_string", "reinitialize store_defaults")
    ...
    def build_build_menu(self, parent_menu):
        ...
        parent_menu.addSeparator()
        self.add_action(parent_menu, "Cycle Bond Valence [Ctrl-Shift-W]", "cmd_string", "cycle_valence")
        self.add_action(parent_menu, "Fill Hydrogens on (pk1) [Ctrl-Shift-R]", "cmd_string", "h_fill")
        self.add_action(parent_menu, "Invert (pk2)-(pk1)-(pk3) [Ctrl-Shift-E]", "cmd_string", "invert")
        self.add_action(parent_menu, "Create Bond (pk1)-(pk2) [Ctrl-Shift-T]", "cmd_string", "bond")
        parent_menu.addSeparator()
        self.add_action(parent_menu, "Remove (pk1) [Ctrl-Shift-D]", "remove", "pk1")
        parent_menu.addSeparator()
        self.add_action(parent_menu, "Make (pk1) Positive [Ctrl-Shift-K]", "alter", ("pk1", "formal_charge=1"))
        self.add_action(parent_menu, "Make (pk1) Negative [Ctrl-Shift-J]", "alter", ("pk1", "formal_charge=-1"))
        self.add_action(parent_menu, "Make (pk1) Neutral [Ctrl-Shift-U]", "alter", ("pk1", "formal_charge=0"))

    def build_movie_menu(self, parent_menu):
        durations = [0.25, 0.5, 1, 2, 3, 4, 6, 8, 12, 18, 24, 30, 48, 60]
        fps_modes = [("30 FPS", 30.0), ("15 FPS", 15.0), ("5 FPS", 5.0), ("1 FPS", 1.0), ("0.3 FPS", 0.3)]
        ...
        self.add_action(parent_menu, "Update Last Program", "cmd_string", "mvprg")
        self.add_action(parent_menu, "Remove Last Program", "cmd_string", "mvprg_remove_last")
        parent_menu.addSeparator()
        ...
        parent_menu.addSeparator()
        self.add_check_action(parent_menu, "Auto Interpolate", "movie_auto_interpolate")
        ...
        self.add_check_action(parent_menu, "Ray Trace Frames", "ray_trace_frames")
        self.add_check_action(parent_menu, "Cache Frame Images", "cache_frames")
        self.add_action(parent_menu, "Clear Image Cache", "cmd_string", "mclear")
        parent_menu.addSeparator()
        self.add_check_action(parent_menu, "Static Singletons", "static_singletons", 1)
        self.add_check_action(parent_menu, "Show All States", "all_states", 1)

    def build_display_menu(self, parent_menu):
        seq_formats = [("Residue Codes", 0), ("Residue Names", 1), ("Chain Identifiers", 3), ("Atom Names", 2), ("States", 4)]
        seq_labels = [("All Residue Numbers", 2), ("Top Sequence Only", 1), ("Object Names Only", 0), ("No Labels", 3)]
        seq_gaps = [("No Gaps", 0), ("All Gaps", 1), ("Single Gap", 2)]
        bg_colors = [("White", 0), ("Light Grey", 134), ("Grey", 104), ("Black", 1)]
        grid_modes = [("By Object", 1), ("By State", 2), ("By Object-State", 3), ("Disable", 0)]
        ...
        self.add_check_action(parent_menu, "Internal Prompt", "internal_prompt", 1)
        self.add_radio_group(parent_menu.addMenu("Internal Feedback"), [("0", 0), ("1", 1), ("3", 3), ("5", 5)], "internal_feedback")
        self.add_radio_group(parent_menu.addMenu("Overlay"), [("0", 0), ("1", 1), ("3", 3), ("5", 5)], "overlay")

        parent_menu.addSeparator()
        self.add_check_action(parent_menu, "Stereo", "stereo", 1)
        stereo_menu = parent_menu.addMenu("Stereo Mode")
        for lab, s_cmd in [("Anaglyph Stereo", "stereo anaglyph"), ("Cross-Eye Stereo", "stereo crosseye"), ("Wall-Eye Stereo", "stereo walleye"), ("Quad-Buffered Stereo", "stereo quadbuffer"), ("Zalman Stereo", "stereo byrow"), ("OpenVR", "stereo openvr")]:
        ...
        stereo_menu.addSeparator()
        self.add_action(stereo_menu, "Chromadepth", "cmd_string", "stereo chromadepth")
        self.add_action(stereo_menu, "off", "cmd_string", "stereo off")

        parent_menu.addSeparator()
        zoom_menu = parent_menu.addMenu("Zoom")
        for ang in [4, 6, 8, 12, 20]:
            self.add_action(zoom_menu, f"{ang} Angstrom Sphere", "zoom_sphere", ang)
        self.add_action(zoom_menu, "All", "cmd_string", "zoom animate=-1")
        self.add_action(zoom_menu, "Complete", "cmd_string", "zoom animate=-1, complete=1")

        clip_menu = parent_menu.addMenu("Clip")
        self.add_action(clip_menu, "Nothing", "cmd_string", "clip atoms, 5, all")
        for slab in [8, 12, 16, 20, 30]:
            self.add_action(clip_menu, f"{slab} Angstrom Slab", "clip_slab", slab)
        ...
        for lab, cs_cmd in [("CMYK (for publications)", "space cmyk"), ("PyMOL (for video + web)", "space pymol"), ("RGB (default)", "space rgb")]:
            self.add_action(cspace_menu, lab, "cmd_string", cs_cmd)
        qual_menu = parent_menu.addMenu("Quality")
        for lab, pq_cmd in [("Maximum Performance", "util.performance(100)"), ("Reasonable Performance", "util.performance(66)"), ("Reasonable Quality", "util.performance(33)"), ("Maximum Quality", "util.performance(0)")]:
            self.add_action(qual_menu, lab, "cmd_string", pq_cmd)

        self.add_radio_group(parent_menu.addMenu("Grid"), grid_modes, "grid_mode")
        parent_menu.addSeparator()
        ...
        self.add_check_action(parent_menu, "Depth Cue (Fogging)", "depth_cue", 1)
        self.add_check_action(parent_menu, "Two Sided Lighting", "two_sided_lighting", 1)
        self.add_check_action(parent_menu, "Specular Reflections", "specular", 1.0)
    ...
    def build_setting_menu(self, parent_menu):
        label_fonts = [("Sans", 5), ("Sans Oblique", 6), ("Sans Bold", 7), ("Sans Bold Oblique", 8), ("Serif", 9), ("Serif Oblique", 17), ("Serif Bold", 10), ("Serif Bold Oblique", 18), ("Mono", 11), ("Mono Oblique", 12), ("Mono Bold", 13), ("Mono Bold Oblique", 14), ("Gentium Roman", 15), ("Gentium Italic", 16)]
        self.add_action(parent_menu, "Edit All...", "cmd_string", "edit_settings")
        self.add_action(parent_menu, "Keyboard Shortcuts...", "cmd_string", "edit_shortcuts")
        ...
        self.add_radio_group(size_menu, [(f"{v} Point", v) for v in [10, 14, 18, 24, 36, 48, 72]], "label_size")
        size_menu.addSeparator()
        self.add_radio_group(size_menu, [(f"{v} Angstrom", -v) for v in [0.3, 0.5, 1, 2, 4]], "label_size")
        self.add_radio_group(lbl_menu.addMenu("Font"), label_fonts, "label_font_id")
        self.add_radio_group(lbl_menu.addMenu("Color"), [("Front", -6), ("Back", -7)], "label_color")
        self.add_check_action(lbl_menu, "Show Connectors", "label_connector")
        self.add_radio_group(lbl_menu.addMenu("Background Color"), [("None", -1), ("Back", -7), ("Front", -6)], "label_bg_color")
        ...
        self.add_check_action(vol_menu, "Pre-integrated Rendering", "volume_mode")
        self.add_radio_group(vol_menu.addMenu("Number of Layers"), [(f"{v:.0f}", v) for v in [100.0, 256.0, 500.0, 1000.0]], "volume_layers")

        trans_menu = parent_menu.addMenu("Transparency")
        t_opts = [("Off", 0.0), ("20%", 0.2), ("40%", 0.4), ("50%", 0.5), ("60%", 0.6), ("80%", 0.8)]
        self.add_radio_group(trans_menu.addMenu("Surface"), t_opts, "transparency")
        self.add_radio_group(trans_menu.addMenu("Sphere"), t_opts, "sphere_transparency")
        self.add_radio_group(trans_menu.addMenu("Cartoon"), t_opts, "cartoon_transparency")
        self.add_radio_group(trans_menu.addMenu("Stick"), t_opts, "stick_transparency")
        trans_menu.addSeparator()
        for lab, values in [("Uni-Layer", (2, 1, 0)), ("Multi-Layer", (1, 0, 1)), ("Multi-Layer (Real-time OIT)", (3, 0, -1)), ("Fast and Ugly", (0, 1, 0))]:
            self.add_action(trans_menu, lab, "transparency_combo", values)
        trans_menu.addSeparator()
        self.add_check_action(trans_menu, "Angle-dependent", "ray_transparency_oblique", 1.0)

        self._build_rendering_layout(parent_menu.addMenu("Rendering"))
        parent_menu.addSeparator()
        self.add_check_action(parent_menu.addMenu("PDB File Loading"), "Ignore PDB Segment Identifier", "ignore_pdb_segi", 1)

        cif_menu = parent_menu.addMenu("mmCIF File Loading")
        self.add_check_action(cif_menu, 'Use "auth" Identifiers', "cif_use_auth", 1)
        self.add_check_action(cif_menu, "Load Assembly (Biological Unit)", "assembly", "1", "")
        self.add_check_action(cif_menu, 'Bonding by "Chemical Component Dictionary"', "connect_mode", 4, 0)

        map_menu = parent_menu.addMenu("Map File Loading")
        self.add_check_action(map_menu, "Normalize CCP4 Maps", "normalize_ccp4_maps", 1)
        self.add_check_action(map_menu, "Normalize O Maps", "normalize_o_maps", 1)

        parent_menu.addSeparator()
        auto_menu = parent_menu.addMenu("Auto-Show ...")
        self.add_check_action(auto_menu, "Cartoon/Sticks/Spheres by Classification", "auto_show_classified", -1, 0)
        auto_menu.addSeparator()
        self.add_check_action(auto_menu, "Auto-Show Lines", "auto_show_lines", 1)
        self.add_check_action(auto_menu, "Auto-Show Spheres", "auto_show_spheres", 1)
        self.add_check_action(auto_menu, "Auto-Show Nonbonded", "auto_show_nonbonded", 1)
        auto_menu.addSeparator()
        self.add_check_action(auto_menu, "Auto-Show New Selections", "auto_show_selections", 1)
        self.add_check_action(auto_menu, "Auto-Hide Selections", "auto_hide_selections", 1)

        self.add_check_action(parent_menu, "Auto-Zoom New Objects", "auto_zoom", 1)
        self.add_check_action(parent_menu, "Auto-Remove Hydrogens", "auto_remove_hydrogens", 1)
        parent_menu.addSeparator()
    ...
    def build_scene_menu(self, parent_menu):
        ...
        parent_menu.addSeparator()
        self.add_action(parent_menu, "Next [PgDn]", "scene_action", ("", "next"))
        self.add_action(parent_menu, "Previous [PgUp]", "scene_action", ("", "previous"))
        parent_menu.addSeparator()
        self.add_action(parent_menu, "Append", "scene_action", ("new", "store"))

        app_menu = parent_menu.addMenu("Append...")
        for lab, param in [("Camera", "scene new, store, color=0, rep=0"), ("Color", "scene new, store, view=0, rep=0"), ("Reps", "scene new, store, view=0, color=0"), ("Reps + Color", "scene new, store, view=0")]:
            self.add_action(app_menu, lab, "cmd_string", param)
        self.add_action(parent_menu, "Insert Before", "scene_action", ("", "insert_before"))
        self.add_action(parent_menu, "Insert After", "scene_action", ("", "insert_after"))
        self.add_action(parent_menu, "Update", "scene_action", ("auto", "update"))
        parent_menu.addSeparator()
        self.add_action(parent_menu, "Delete", "scene_action", ("auto", "clear"))
        parent_menu.addSeparator()
        ...
        for lab, mode in [("Enable", "enable"), ("Optimize", "optimize"), ("Read Only", "read_only"), ("Disable", "disable")]:
            self.add_action(cache_menu, lab, "scene_cache", mode)

    def build_mouse_menu(self, parent_menu):
        self.add_radio_group(parent_menu.addMenu("Selection Mode"), [("Atoms", 0), ("Residues", 1), ("Chains", 2), ("Segments", 3), ("Objects", 4), ("Molecules", 5), ("C-alphas", 6)], "mouse_selection_mode")
        parent_menu.addSeparator()
        for lab, mode in [("3 Button Motions", "three_button_motions"), ("3 Button Editing", "three_button_editing"), ("3 Button Viewing", "three_button_viewing"), ("3 Button Lights", "three_button_lights"), ("3 Button All Modes", "three_button_all_modes"), ("2 Button Editing", "two_button_editing"), ("2 Button Viewing", "two_button"), ("1 Button Viewing Mode", "one_button_viewing"), ("Emulate Maestro", "three_button_maestro")]:
            self.add_action(parent_menu, lab, "mouse_config", mode)
        parent_menu.addSeparator()
        self.add_check_action(parent_menu, "Virtual Trackball", "virtual_trackball")
    ...
    def build_wizard_menu(self, parent_menu):
        ...
        for lab, val in [("Representations", "reps"), ("Cartoon Ribbons", "cartoon"), ("Roving Detail", "roving"), ("Roving Density", "roving_density"), ("Transparency", "trans"), ("Ray Tracing", "ray"), ("Sculpting", "sculpt"), ("Scripted Animation", "anime"), ("Electrostatics", "elec"), ("Compiled Graphics Objects", "cgo"), ("Molscript/Raster3D Input", "raster3d")]:
    ...
    def build_help_menu(self, parent_menu):
        self.add_action(parent_menu, "PyMOL Command Reference", "url", "http://pymol.org/pymol-command-ref.html")
        parent_menu.addSeparator()
        self.add_action(parent_menu, "Online Documentation", "url", "http://pymol.org/d/")
        topics_menu = parent_menu.addMenu("Topics")
        for lab, topic in [("Introductory Screencasts", "media:intro"), ("Core Commands", "command:core_set"), ("Settings", "setting"), ("Atom Selections", "selection"), ("Commands", "command"), ("Launching", "launch"), ("Concepts", "concept"), ("A.P.I. Methods", "api")]:
            self.add_action(topics_menu, lab, "help_topic", topic)
        parent_menu.addSeparator()
        for lab, target_url in [("PyMOL Community Wiki", "http://www.pymolwiki.org"), ("PyMOL Mailing List", "https://lists.sourceforge.net/lists/listinfo/pymol-users"), ("PyMOL Home Page", "http://www.pymol.org")]:
            self.add_action(parent_menu, lab, "url", target_url)
        parent_menu.addSeparator()
        self.add_action(parent_menu, "About PyMOL", "cmd_string", "about")
        self.add_action(parent_menu, "Sponsorship Information", "url", "http://pymol.org/funding.html")
        self.add_action(parent_menu, "How to Cite PyMOL", "url", "http://pymol.org/citing")

    def _build_fragments_layout(self, menu):
        for lab, f_name, atom, mode in [("Acetylene [Alt-J]", 'acetylene', 2, 0), ("Amide N->C [Alt-1]", 'formamide', 3, 1), ("Amide C->N [Alt-2]", 'formamide', 5, 0), ("Carbonyl [Alt-0]", 'formaldehyde', 2, 0), ("Cyclobutyl [Alt-4]", 'cyclobutane', 4, 0), ("Cyclopentyl [Alt-5]", 'cyclopentane', 5, 0), ("Cyclopentadiene [Alt-8]", 'cyclopentadiene', 5, 0), ("Cyclohexyl [Alt-6]", 'cyclohexane', 7, 0), ("Cycloheptyl [Alt-7]", 'cycloheptane', 8, 0), ("Methane [Ctrl-Shift-M]", 'methane', 1, 0), ("Sulfonyl [Alt-3]", 'sulfone', 3, 1)]:
            self.add_action(menu, lab, "attach_fragment", (f_name, atom, mode))
        menu.addSeparator()
        for lab, elem, geom, val in [("Bromine [Ctrl-Shift-B]", 'Br', 1, 1), ("Carbon [Ctrl-Shift-C]", 'C', 4, 4), ("Chlorine [Ctrl-Shift-L]", 'Cl', 1, 1), ("Fluorine [Ctrl-Shift-F]", 'F', 1, 1), ("Iodine [Ctrl-Shift-I]", 'I', 1, 1), ("Nitrogen [Ctrl-Shift-N]", 'N', 4, 3), ("Oxygen [Ctrl-Shift-O]", 'O', 4, 2), ("Sulfer [Ctrl-Shift-S]", 'S', 2, 2), ("Phosphorus [Ctrl-Shift-P]", 'P', 4, 3)]:
            self.add_action(menu, lab, "replace_element", (elem, geom, val))

    def _build_residues_layout(self, menu):
        for lab, r_name in [("Acetyl [Alt-B]", "ace"), ("Alanine [Alt-A]", "ala"), ("Amine", "nhh"), ("Aspartate [Alt-D]", "asp"), ("Asparagine [Alt-N]", "asn"), ("Arginine [Alt-R]", "arg"), ("Cysteine [Alt-C]", "cys"), ("Glutamate [Alt-E]", "glu"), ("Glutamine [Alt-Q]", "gln"), ("Glycine [Alt-G]", "gly"), ("Histidine [Alt-H]", "his"), ("Isoleucine [Alt-I]", "ile"), ("Leucine [Alt-L]", "leu"), ("Lysine [Alt-K]", "lys"), ("Methionine [Alt-M]", "met"), ("N-Methyl [Alt-Z]", "nme"), ("Phenylalanine [Alt-F]", "phe"), ("Proline [Alt-P]", "pro"), ("Serine [Alt-S]", "ser"), ("Threonine [Alt-T]", "thr"), ("Tryptophan [Alt-W]", "trp"), ("Tyrosine [Alt-Y]", "tyr"), ("Valine [Alt-V]", "val")]:
            self.add_action(menu, lab, "attach_amino_acid", r_name)
        menu.addSeparator()
        self.add_radio_group(menu, [("Helix", 1), ("Antiparallel Beta Sheet", 2), ("Parallel Beta Sheet", 3)], "secondary_structure")
    ...
    def _build_sculpting_layout(self, menu):
        ...
        self.add_action(menu, "Deactivate", "cmd_string", "sculpt_deactivate all")
        self.add_action(menu, "Clear Memory", "cmd_string", "sculpt_purge")
        menu.addSeparator()
        self.add_radio_group(menu, [("1 Cycle per Update", 1)] + [(f"{v} Cycles per Update", v) for v in [3, 10, 33, 100, 333, 1000]], "sculpting_cycles")
        menu.addSeparator()
        self.add_radio_group(menu, [("Bonds Only", 0x01), ("Bonds and Angles Only", 0x01 | 0x02), ("Local Geometry Only", 0x20 - 1), ("All Except VDW", ~(0x20 | 0x40)), ("All Except 1-4 VDW and Torsions", ~(0x40 | 0x80)), ("All Terms", 0xFF)], "sculpt_field_mask")

    def _build_movie_programs_layout(self, menu):
        cam_loop = menu.addMenu("Camera Loop")
        nutate_menu = cam_loop.addMenu("Nutate")
        for lab, s_expr in [("15 deg. over 4 sec.", "movie.add_nutate(4,15,start=%d)"), ("15 deg. over 8 sec.", "movie.add_nutate(8,15,start=%d)"), ("15 deg. over 12 sec.", "movie.add_nutate(12,15,start=%d)"), ("30 deg. over 4 sec.", "movie.add_nutate(4,30,start=%d)"), ("30 deg. over 8 sec.", "movie.add_nutate(8,30,start=%d)"), ("30 deg. over 12 sec.", "movie.add_nutate(12,30,start=%d)"), ("30 deg. over 16 sec.", "movie.add_nutate(16,30,start=%d)"), ("60 deg. over 8 sec.", "movie.add_nutate(8,60,start=%d)"), ("60 deg. over 16 sec.", "movie.add_nutate(16,60,start=%d)"), ("60 deg. over 24 sec.", "movie.add_nutate(24,60,start=%d)"), ("60 deg. over 32 sec.", "movie.add_nutate(32,60,start=%d)")]:
            self.add_action(nutate_menu, lab, "cmd_string", s_expr)
        for axis in ['x', 'y']:
            rock_m = cam_loop.addMenu(f"{axis.upper()}-Rock")
            for deg, seconds in [(30, (2, 4, 8)), (60, (4, 8, 16)), (90, (6, 12, 24)), (120, (8, 16, 32)), (180, (12, 24, 48))]:
                for s in seconds:
                    val = 179.99 if deg == 180 else deg
                    self.add_action(rock_m, f"{deg} deg. over {s} sec.", "cmd_string", f"movie.add_rock({s},{val},axis='{axis}',start=%d)")
            roll_m = cam_loop.addMenu(f"{axis.upper()}-Roll")
            for s in [4, 8, 16, 32]:
                self.add_action(roll_m, f"{s} seconds", "cmd_string", f"movie.add_roll({s}.0,axis='{axis}',start=%d)")
        menu.addSeparator()
        ...
        for type_lab, r_val in [("Nutate", 4), ("X-Rock", 2), ("Y-Rock", 1)]:
            ...
            for angle, seconds in ((30, (2, 4, 8)), (60, (4, 8, 16)), (90, (6, 12, 24)), (120, (8, 16, 32))):
                for sec in seconds:
                    self.add_action(sl_m, f"{angle} deg. over {sec} sec.", "cmd_string", f"set sweep_angle,{angle};cmd.movie.add_scenes(None, {sec}, rock={r_val}, start=%d)")
        steady_m = scene_loop.addMenu("Steady")
        for val in [1, 2, 4, 8, 12, 16, 24]:
            self.add_action(steady_m, f"{val} seconds each", "cmd_string", f"movie.add_scenes(None,{val:.1f},rock=0,start=%d)")
        menu.addSeparator()
        for loop_lab, fmt in [("State Loop", "movie.add_state_loop(%d, %d"), ("State Sweep", "movie.add_state_sweep(%d, %d")]:
            l_menu = menu.addMenu(loop_lab)
            for speed in [1, 2, 3, 4, 8, 16]:
                sp_m = l_menu.addMenu("Full Speed" if speed == 1 else f"1/{speed} Speed")
                for pause in [0, 1, 2, 4]:
                    self.add_action(sp_m, f"{pause} second pause" if pause else "no pause", "cmd_string", (fmt % (speed, pause)) + ", start=%d)")

    def _build_lines_sticks_layout(self, menu):
        self.add_check_action(menu, "Ball and Stick", "stick_ball", 1)
        self.add_radio_group(menu.addMenu("Ball and Stick Ratio"), [("1.0", 1.0), ("1.5", 1.5), ("VDW", -1.0)], "stick_ball_ratio")
        menu.addSeparator()
        self.add_radio_group(menu.addMenu("Zero Order Bonds"), [("Hide", 0), ("Dashed", 1), ("Solid", 2)], "valence_zero_mode")
        self.add_radio_group(menu.addMenu("Zero Order Stick Scale"), [("0.1", 0.1), ("0.2", 0.2), ("0.3", 0.3), ("1.0", 1.0)], "valence_zero_scale")
        menu.addSeparator()
        self.add_radio_group(menu.addMenu("Stick Radius"), [("0.1", 0.1), ("0.2", 0.2), ("0.25", 0.25)], "stick_radius")
        self.add_radio_group(menu.addMenu("Stick Hydrogen Scale"), [("0.4", 0.4), ("1.0", 1.0)], "stick_h_scale")
        menu.addSeparator()
        self.add_radio_group(menu.addMenu("Line Width"), [("1.0", 1.0), ("1.49", 1.49), ("3.0", 3.0)], "line_width")
        self.add_check_action(menu, "Lines As Cylinders", "line_as_cylinders", 1)

    def _build_cartoon_layout(self, menu):
        r_menu = menu.addMenu("Rings and Bases")
        self.add_radio_group(r_menu, [("Filled Rings (Round Edges)", 1), ("Filled Rings (Flat Edges)", 2), ("Filled Rings (with Border)", 3), ("Spheres", 4), ("Base Ladders", 0)], "cartoon_ring_mode")
        r_menu.addSeparator()
        self.add_radio_group(r_menu, [("Bases and Sugars", 1), ("Bases Only", 2), ("Non-protein Rings", 3), ("All Rings", 4)], "cartoon_ring_finder")
        r_menu.addSeparator()
        self.add_radio_group(r_menu, [("Transparent Rings", 0.5), ("Default", -1)], "cartoon_ring_transparency")
        self.add_check_action(menu, "Side Chain Helper", "cartoon_side_chain_helper", 1)
        ...
        self.add_check_action(menu, "Cylindrical Helices", "cartoon_cylindrical_helices", 1)
        ...
        self.add_check_action(menu, "Discrete Colors", "cartoon_discrete_colors", 1)
        self.add_check_action(menu, "Highlight Color", "cartoon_highlight_color", 104, -1)
        self.add_radio_group(menu.addMenu("Sampling"), [("Atom count dependent", -1), ("2", 2), ("7", 7), ("14", 14)], "cartoon_sampling")
        self.add_radio_group(menu.addMenu("Gap Cutoff"), [("0", 0), ("5", 5), ("10", 10), ("20", 20)], "cartoon_gap_cutoff")

    def _build_ribbon_layout(self, menu):
        self.add_check_action(menu, "Side Chain Helper", "ribbon_side_chain_helper", 1)
        self.add_check_action(menu, "Trace Atoms", "ribbon_trace_atoms", 1)
        menu.addSeparator()
        self.add_radio_group(menu, [("As Lines", 0), ("As Cylinders", 1)], "ribbon_as_cylinders")
        self.add_radio_group(menu.addMenu("Cylinder Radius"), [("Match Line Width", 0.0), ("0.2 Angstrom", 0.2), ("0.5 Angstrom", 0.5), ("1.0 Angstrom", 1.0)], "ribbon_radius")

    def _build_surface_layout(self, menu):
        self.add_radio_group(menu.addMenu("Color"), [("White", 0), ("Light Gray", 4236), ("Gray", 25), ("Default (Atomic)", -1)], "surface_color")
        self.add_radio_group(menu, [("Dot", 1), ("Wireframe", 2), ("Solid", 0)], "surface_type")
        menu.addSeparator()
        self.add_radio_group(menu, [("Cavities and Pockets Only", 1), ("Cavities and Pockets (Culled)", 2)], "surface_cavity_mode")
        self.add_radio_group(menu.addMenu("Cavity Detection Radius"), [("7 Angstrom", 7)] + [(f"{v} Solvent Radii", -v) for v in [3, 4, 5, 6, 8, 10, 20]], "surface_cavity_radius")
        self.add_radio_group(menu.addMenu("Cavity Detection Cutoff"), [(f"{v} Solvent Radii", -v) for v in [1, 2, 3, 4, 5]], "surface_cavity_cutoff")
        self.add_radio_group(menu, [("Exterior (Normal)", 0)], "surface_cavity_mode")
        menu.addSeparator()
        ...
        menu.addSeparator()
        self.add_radio_group(menu, [("Ignore None", 1), ("Ignore HETATMs", 0), ("Ignore Hydrogens", 2), ("Ignore Unsurfaced", 3)], "surface_mode")
    ...
    def _build_rendering_layout(self, menu):
        ...
        self.add_radio_group(menu.addMenu("Antialias (Real Time)"), [("off", 0), ("FXAA", 1), ("SMAA", 2)], "antialias_shader")
        menu.addSeparator()
        self.add_action(menu, "Modernize", "modernize_rendering")
        menu.addSeparator()
        shd_menu = menu.addMenu("Shadows")
        for val in ["none", "light", "medium", "heavy", "black", "matte", "soft", "occlusion", "occlusion2"]:
            self.add_action(shd_menu, val.title(), "ray_shadows", val)
        tex_opts = [("None", 0), ("Matte 1", 1), ("Matte 2", 4), ("Swirl 1", 2), ("Swirl 2", 3), ("Fiber", 5)]
        self.add_radio_group(menu.addMenu("Texture"), tex_opts, "ray_texture")
        self.add_radio_group(menu.addMenu("Interior Texture"), tex_opts, "ray_interior_texture")
        self.add_radio_group(menu.addMenu("Memory"), [("Use Less (slower)", 70), ("Use Standard Amount", 100), ("Use More (faster)", 170), ("Use Even More", 230), ("Use Most", 300)], "hash_max")
        menu.addSeparator()
        self.add_check_action(menu, "Cull Backfaces", "backface_cull", 1)
        self.add_check_action(menu, "Opaque Interiors", "ray_interior_color", 74, -1)

    def _build_fkeys_scene_menu(self, menu, action_name):
        for i in range(1, 13):
            self.add_action(menu, f"F{i}", "scene_action", (f"F{i}", action_name))
# </editor-fold>
