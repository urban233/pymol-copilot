#!/usr/bin/env python3
# PyMOL Copilot - open-source AI assistant for the PyMOL computer program
# -------------------------------------------------------------------
# This file contains source code for the PyMOL Copilot computer program
# Copyright (C) 2026 Hannah Kullik, Martin Urban (hannah.kullik@studmail.w-hs.de, martin.urban@studmail.w-hs.de)
# Source code is available at <https://github.com/urban233/pymol-copilot>
# -------------------------------------------------------------------
...
@task
def format(dry_run: str = "false") -> None:
    """Format the codebase with ruff.

    Runs ruff in either reformat or check-only mode (dry run).

    Args:
        dry_run: Pass check=true to run ruff format in --check mode (exit
    ...
    Example:
        ...
        ./pymake.bat/sh format dry_run=true # CI dry-run; fail if changes needed.
    """
    flag = "--check" if dry_run.lower() == "true" else ""
...
def lint(fix: str = "false") -> None:
    ...
    Executes ruff across the entire project
    tree using the configuration defined in ruff.toml.

    Args:
        fix: Pass fix=true to automatically fix lint violations where possible.
            Defaults to "false".
    """
...
@task
def docs(output: str = "docs/api") -> None:
    """Generate API documentation using pdoc.

    Args:
        output: Directory where the documentation will be generated.
            Defaults to "docs/api".
    """
    print(
        _colorize(f"Generating API documentation to {output}...", _COLOR_GREEN)
    )
    env = os.environ.copy()
    src_paths = [
        os.path.abspath("src/python"),
        os.path.abspath("plugin_builder/src"),
    ]
    ...
    packages = [
        ...
        # "src/python/pymol",
        # "src/python/pymol2",
        # "plugin_builder/src/plugin_builder",
    ]
    existing_packages = [pkg for pkg in packages if pathlib.Path(pkg).exists()]
    cmd = f"pdoc -o {output} {' '.join(existing_packages)}"
    run(cmd, env=env)
...
def build_whl() -> None:
    ...
    if is_windows:
        ...
        py_lib_dir = (
            py_lib_dirs[0] if py_lib_dirs else (env_dir / "lib" / "python3")
        )
    ...
    print(
        _colorize(
            f"Detected OS: {'Windows' if is_windows else 'POSIX (Linux/macOS)'}",
            _COLOR_GREY,
        )
    )
    ...
    print(
        _colorize(
            "Wheel build complete. Check the 'dist/' directory.", _COLOR_GREEN
        )
    )
