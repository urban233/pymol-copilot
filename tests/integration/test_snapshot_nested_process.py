...
"""Meta-test for the shared nested-subprocess snapshot test support.

`run_nested_snapshot_process` (snapshot_support.py) is what
test_snapshot_round_trip.py relies on to learn whether reconstruction in a
genuinely separate process actually matched the original. It launches the
nested process through the same `pytest.main()` -> flush -> `os._exit()`
sequence every real-PyMOL test module's own `__main__` block uses,
specifically so real PyMOL's headless shutdown can never override a
genuine nested failure's exit code with 0 (see that function's own
docstring for the full empirical finding this fixes).

This module tests that guarantee directly, independent of the snapshot
round trip's own reconstruction logic -- but it cannot do so without real
...
round-trip reconstruction test does before it fails, reproducing the exact
masking condition run_nested_snapshot_process's own docstring documents.
That makes this module's own test a genuine regression guard: temporarily
reverting run_nested_snapshot_process's spawn back to `python -m pytest`
makes this test fail, exactly as a guard for that regression must.
"""
...
#: point every round-trip reconstruction test does, and only then
...
_FAILING_TEST_SOURCE = (
    ...
    "    cmd.sync()\n"
    "    raise AssertionError('snapshot harness meta-test induced failure')\n"
)
...
def test_run_nested_snapshot_process_propagates_a_genuine_failure(
    ...
    Raises:
        AssertionError: If the nested failure's exit code was masked back
            ...
            captured output.
    """
    failing_test_file = tmp_path / "test_snapshot_harness_deliberate_failure.py"
...
if __name__ == "__main__":
    ...
    # and test_snapshot_round_trip.py's own __main__ block). os._exit
    # bypasses that interpreter-shutdown window entirely, so pytest's real
    # result is what Bazel actually sees. os._exit skips the normal stdio
    # flush, so flush explicitly first -- otherwise a real failure's
    # traceback and summary can be silently lost from the captured test
    # log.
