...
"""Hermetic evidence for the frozen colour name-to-index table.
...
without PyMOL is that the table still covers exactly the colour names
the command language accepts, which is the property the oracle relies on
when it predicts a colour it was handed by a plan.
"""
...
def test_table_covers_the_whole_allowlist() -> None:
    """Every accepted colour must be predictable without PyMOL.
...
def test_no_color_maps_to_an_unresolved_index() -> None:
    """PyMOL reports -1 for a colour it does not know.

    Freezing a -1 would record "unknown" as though it were a real index
    and let a colour assertion pass against an atom PyMOL never coloured.
    """
    assert [
